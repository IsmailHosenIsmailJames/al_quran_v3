...
class _QiblaView extends StatelessWidget {
            ...
            >(
              listener: (context, locationState) {
                if (locationState.kaabaAngle != null) {
                  context.read<QiblaCubit>().updateKaabaAngle(
                    locationState.kaabaAngle!,
                  );
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          _buildMetricColumn(
            label: l10n.heading,
            value: "${localizedNumber(context, heading.round())}°",
          ...
          _buildMetricColumn(
            label: l10n.qibla,
            value: "${localizedNumber(context, kaabaAngle.round())}°",
            color: themeState.primary,
          ),
        ],
      ),
  ...
  Widget _buildAlignmentBadge({
    ...
    if (isAligned) {
      guidanceText = l10n.alignedWithKaaba;
      iconData = Icons.check_circle_rounded;
    } else if (diff > 0) {
      guidanceText = l10n.turnRight(
        localizedNumber(context, diff.abs().round()),
      );
      iconData = Icons.arrow_circle_right_rounded;
    } else {
      guidanceText = l10n.turnLeft(
        localizedNumber(context, diff.abs().round()),
      );
