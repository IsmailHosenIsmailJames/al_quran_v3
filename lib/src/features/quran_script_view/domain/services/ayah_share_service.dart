...
class AyahShareService {
  ...
  static Future<String> generateAyahsShareText({
    ...
    // Sort ayah keys chronologically
    ...
    for (final ayahKey in sortedKeys) {
      ...
      // Format Ayah Text
      buffer.writeln("$surahName • Ayah $ayahIdStr");
      buffer.writeln();
      buffer.writeln(plainArabicText);
      buffer.writeln();
      buffer.writeln("— Shared from Al Quran Audio");
      buffer.writeln("----------------------------------");
      buffer.writeln();
    }
  ...
  /// Triggers standard system share dialog for the given Ayahs.
