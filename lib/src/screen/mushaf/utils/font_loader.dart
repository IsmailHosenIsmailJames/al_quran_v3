...
class DynamicFontLoader {
  ...
  /// Loads a font dynamically from a local file path.
  static Future<String> loadLocalFont(
    String fontFamily,
    String filePath,
  ) async {
    if (_loadedFonts.contains(fontFamily)) {
    ...
    if (!await fontFile.exists()) {
      log("Font file not found: $filePath", name: "DynamicFontLoader");
      return "QPC_Hafs";
    }
    ...
    try {
      ...
      log(
        "Error loading local font $fontFamily: $e",
        name: "DynamicFontLoader",
      );
      return "QPC_Hafs";
    }
  }
